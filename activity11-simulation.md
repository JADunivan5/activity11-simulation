...
``` r
Exp_1000 <- exp_samp_1000
  mean(Exp_1000$values)
```
...
generate_n_samples <- function(n){
  tibble(
  Normal = rnorm(n,mean=10,sd=5),
  Exponential = rexp(n, rate=2.5),
  Chi_square = rchisq(n, df=55, ncp= 12.5),
  Uniform = runif(n, min= 5, max=25)
  )
}
```

``` r
Multiple_Samples_1000 <- generate_n_samples(n=1000)
Multiple_Samples_30 <- generate_n_samples(n=30)
```
...
``` r
Mean_1000 <- sample_mean(.data = Multiple_Samples_1000)
Mean_30 <- sample_mean(.data = Multiple_Samples_30)                    
#Means with a larger sample are going to be more accurate, than means of 30                     
```
...
![](activity11-simulation_files/figure-gfm/unnamed-chunk-1-1.png)<!-- -->
    ...
    ##   distribution      mean    sd
    ##   <chr>            <dbl> <dbl>
    ## 1 mean_binomial    25.0  0.343
    ## 2 mean_exponential  4.99 0.399
    ## 3 mean_normal      50.0  0.812
    ## 4 mean_uniform     15.0  0.281
